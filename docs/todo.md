...
- [x] フェーズ0: PoC（pyan3/code2flow/pydeps/gprof2dot を導入し実コードで検証、採否確定、knowledge.md に記録）※3.11コンテナで実施、3.12実機確認は残
- [x] フェーズ0: 自前 _CallCollector の撤去可否を pyan3 のファイル横断解決の実測で判断 → C1/C3解消を確認し**方針A（pyan3一本化）を推奨**（knowledge.md 2026-07-13）
- [ ] フェーズ0残: WSL2+Python3.12 実機での pyan3 動作・大規模実コードでの安定性/実行時間の確認
- [x] フェーズ1: pyan3 でファイル横断の静的コールグラフ統合＋起点/深さ絞り込み＋画像書き出し（FR-1〜4）
  - [x] callgraph.py 新設（pyan3→DOT→dotでSVG、text出力、ツール/graphviz未導入フォールバック）
  - [x] app.py に「ファイル横断の呼び出しグラフ」パネル追加（起点関数/向き/粒度/テスト含む、SVG表示＋SVG/DOT/text書き出し）
  - [x] requirements.txt に pyan3 追加、docstring/凡例に限界注記
  - [x] test_callgraph.py 12件＋AppTestスモーク1件。全111件パス（98→111）
- [ ] フェーズ1残: pydeps のモジュール依存図・循環import検出パネル（FR-5, 任意）
